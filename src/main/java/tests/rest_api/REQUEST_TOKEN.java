...
@Slf4j
public class REQUEST_TOKEN extends TestNGCitrusSpringSupport {
    ...
    public void testKafkaUiLogin() {
        ...
        $(http().client(kafkaUiAuthLocal)
                ...
                .type(MessageType.JSON)
                .extract(jsonPath()
                        .expression("$.token", "kafkaUiToken")
                )
                .validate(jsonPath()
                        .expression("$.user.username", "admin")
                        .expression("$.token", "@ignore()@")
                        //.asExtractor("kafkaUiToken", "$.token")
                )
                .build());
        String token = context.getVariable("kafkaUiToken");
