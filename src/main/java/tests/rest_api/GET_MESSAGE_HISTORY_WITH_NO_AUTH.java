...
@Slf4j
public class GET_MESSAGE_HISTORY_WITH_NO_AUTH extends TestNGCitrusSpringSupport {
    ...
    public void testKafkaUiMessagesHistory() {
        ...
        $(http().client(kafkaUiAuthLocal)
                .send()
                .get("/api/messages/history?limit=4")
                .message()
        ...
        $(http().client(kafkaUiAuthLocal)
                ...
                .message()
                .validate(jsonPath()
                        .expression("$.error", "Missing or invalid token")
                )
                .type(MessageType.JSON)
                .build());
    }
